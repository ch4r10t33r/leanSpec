...
class TestAggregatorControllerWrite:
    ...
    async def test_concurrent_toggles_converge(self, peer_id: PeerId) -> None:
        """Concurrent toggles execute serially and converge to the last-writer value."""
        controller, sync_service, network_service = _make_controller(peer_id, initial=False)
        ...
        # Ordering is preserved by the lock, so the final state matches the last call.
